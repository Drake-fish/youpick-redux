...
export default class Result extends Component{
  ...
  let tip=details.tips.groups[0].items.length > 0 ? details.tips.groups[0].items[0].text : null;
  let user=details.tips.groups[0].items.length > 0 ? `${details.tips.groups[0].items[0].user.photo.prefix}original${details.tips.groups[0].items[0].user.photo.suffix}` : null ;
  ...
  let photo=details.photos.groups.length > 0 ? `${details.photos.groups[0].items[0].prefix}original${details.photos.groups[0].items[0].suffix}` : null;
  let url = `https://www.google.com/maps/embed/v1/directions?origin=${location.latitude}%2C${location.longitude}&destination=${details.location.address}&key=${key}`
      ...
      <div>
        ...
        <div className="details-container">
          ...
          <InformationComponent infoOpen={infoOpen} title={details.name} tip={tip} user={user} type={type} cost={cost} rating={details.rating} ratingColor={details.ratingColor} message={message} menu={menu}/>
                ...
                <div className={contactOpen ? "contact contact-open" : "contact"}>
                  ...
                  <div className="hours">
                   ...
                   {details.hours !== undefined && details.hours.timeframes.map((hour,i) => {
                     return (<h5 key={i}>{hour.days} {hour.open[0].renderedTime}</h5>);
                      })
                   }
                  </div>
